...
/*        $query_cdrs = "SELECT table_name from cdrs_list where status='0'";
        $table_name_cdrs = $db->run($query_cdrs);

	//$tablename = "cdrs";
	$tablename = $table_name_cdrs[0]['table_name'];
*/
$logger->log(print_r("table suffix here".$data['tbl_suffix'],true));
	$tablename = "cdrs".$data['tbl_suffix'];
...
//	$query_cdrs_fail = "SELECT table_name from fail_cdrs_list where status='0'";
//        $table_name_cdrs_fail = $db->run($query_cdrs_fail);
	$tablename = "fail_cdrs".$data['tbl_suffix'];
	//$tablename = $data['CDRTABLE']['tbl_fail_cdr']; 
//	$tablename = $table_name_cdrs_fail[0]['table_name'];
    }
	...
	#   Modified By : ITPL
	#   Purpose : Remove unnecessary parameters

	#	$query = "INSERT INTO ".$tablename." (uniqueid,accountid,type,callerid,callednum,billseconds,trunk_id,trunkip,callerip,disposition,callstart,debit,cost,provider_id,pricelist_id,package_id,pattern,notes,rate_cost,reseller_id,reseller_code,reseller_code_destination,reseller_cost,provider_code,provider_code_destination,provider_cost,provider_call_cost,call_direction,calltype,profile_start_stamp,answer_stamp,bridge_stamp,progress_stamp,progress_media_stamp,end_stamp,billmsec,answermsec,waitmsec,progress_mediamsec,flow_billmsec)  values ($cdr_string)";

    $query = "INSERT INTO ".$tablename." (uniqueid,accountid,type,callerid,callednum,billseconds,trunk_id,trunkip,callerip,disposition,callstart,debit,cost,provider_id,pricelist_id,package_id,pattern,notes,rate_cost,reseller_id,reseller_code,reseller_code_destination,reseller_cost,provider_code,provider_code_destination,provider_cost,provider_call_cost,call_direction,calltype)  values ($cdr_string)";
	$logger->log($query);
	$db->run($query);
    ...
    //Resellers CDR entry
    $flag_parent = false;
    insert_parent_data($dataVariable,$actual_calltype,$parentid,$origination_rate,$actual_duration,$provider_cost,$flag_parent,$logger,$db,$decimal_points,$data['tbl_suffix']);
	...
	if(isset($dataVariable['receiver_accid']) && $dataVariable['receiver_accid'] != "")
	{
		...
		insert_extra_receiver_entry($dataVariable,$origination_rate,$termination_rate,$account_type,$actual_duration,$provider_cost,$receiver_parentid,$flag_parent,$dataVariable['receiver_accid'],$logger,$db,$decimal_points,$data['tbl_suffix']);

		$flag_parent = true;
		$dataVariable['uuid'] = $dataVariable['uuid'].$dataVariable['calltype']."_".$receiver_parentid;

        //Insert parent reseller cdr
		insert_parent_data($dataVariable,$actual_calltype,$receiver_parentid,$origination_rate,$actual_duration,$provider_cost,$flag_parent,$logger,$db,$decimal_points,$data['tbl_suffix']);
...
function insert_parent_data($dataVariable,$actual_calltype,$parentid,$origination_rate,$actual_duration,$provider_cost,$flag_parent,$logger,$db,$decimal_points,$tbl_suffix)
{
	while($parentid > 0 )
	{
		...
		if(isset($dataVariable['receiver_accid']) && $dataVariable['receiver_accid'] != "" && $flag_parent == true)
		{
		    ...
		    $cdr_string = get_reseller_cdr_string($dataVariable,$accountid,$account_type,$actual_duration,$termination_rate,$origination_rate,$provider_cost,$parentid,$debit,$cost);	
 			...
 			$query = "INSERT INTO reseller_cdrs".$tbl_suffix." (uniqueid,accountid,callerid,callednum,billseconds,disposition,callstart,debit,cost,pricelist_id,package_id,pattern,notes,rate_cost,
...
function insert_extra_receiver_entry($dataVariable,$origination_rate,$termination_rate,$account_type,$actual_duration,$provider_cost,$parentid,$flag_parent,$accountid,$logger,$db,$decimal_points,$tbl_suffix)
{
	 ...
	 $logger->log("**********************Reseeler CDRs  Table  *************".$CDRtableArr['tbl_reseller_cdr']);
 
		if($flag_parent == false)
		{
			# Modified By : ITPL
			# Purpose : To remove unnecessary Params which are not required.
			# Date : 13-Feb-2017
			
			$query = "INSERT INTO cdrs".$tbl_suffix."(uniqueid,accountid,type,callerid,callednum,billseconds,trunk_id,trunkip,callerip,disposition,callstart,debit,cost,provider_id,pricelist_id,package_id,pattern,notes,rate_cost,reseller_id,reseller_code,reseller_code_destination,reseller_cost,provider_code,provider_code_destination,provider_cost,provider_call_cost,call_direction,calltype) values ($cdr_string)";
